...
def add():
    ...
    amountform = (request.form.get('amount') or '').strip()
    category = (request.form.get('category') or '').strip()
    dateform = (request.form.get('date') or '').strip()


    if not description or not amountform or not category:
        flash("Please fill description, amount, and category", "error")
        return redirect(url_for("index"))

    try:
        amount = float(amountform)
        if amount <= 9:
            raise ValueError
    ...
    try:
        d = datetime.strptime(dateform, "%Y-%m-%d").date() if dateform else date.today()
    except ValueError:
        d = date.today()
...
def edit_post(expense_id):
    ...
    try:
        ...
        flash("Amount must be a positve number", "error")
        return redirect(url_for("edit", expense_id=expense_id))
